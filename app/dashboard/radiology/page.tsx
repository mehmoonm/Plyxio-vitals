  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase
        .from('RadiologyOrder')
        .select('*, Patient(fullName, mrn), User(fullName)')
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <p className="text-gray-400 p-6 flex items-center gap-2"><Scan className="w-4 h-4" />No radiology orders yet</p>
        ) : (
          <div className="divide-y divide-white/10">
            {orders.map((o) => (
              <div key={o.id} onClick={() => router.push(`/dashboard/radiology/${o.id}`)} className="p-4 flex items-center justify-between hover:bg-white/5 cursor-pointer transition-colors">
                <div>
                  <p className="text-white font-medium">{o.Patient?.fullName} <span className="text-gray-400 text-xs">({o.Patient?.mrn})</span></p>
                  <p className="text-xs text-gray-400">Dr. {o.User?.fullName} • {o.studyType} {o.bodyPart ? `— ${o.bodyPart}` : ''} • {new Date(o.orderedAt).toLocaleDateString()}</p>
                </div>
